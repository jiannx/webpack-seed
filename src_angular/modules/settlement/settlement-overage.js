...
app.controller('settleOverageCtrl', function($scope, $state, $rootScope, request, neDialog, neTable) {
  ...
  grid = neTable.create({
    ...
    columnDefs: [
      ...
      { display: '订单号', field: 'order_no', width: 8 },
      { display: '账户余额', field: 'account_balance', width: 10 },
      { display: '账户所有人', field: 'real_name', width: 10 },
      { display: '手机号', field: 'telphone', width: 10 },
      { display: '负责客服', field: 'customer_service_name', width: 10 },
      { display: '提现金额', field: 'amount', width: 10 },
      { display: '提现申请日期', field: 'createtime', width: 10 },
