...
app.controller('orderRechargeMoneyCtrl', function($scope, $state, $rootScope, request, neDialog, neTable) {
  let grid = null;
  let def = {
    source: 2, // 余额充值
    order_id: '',
    ...
    columnDefs: [
      ...
      { display: '状态', field: 'status', width: 10 }, {
        ...
        field: function(rowData) {
          let id = rowData.id;
          let tpl = `<a class="bg-success" ui-sref="index.order.recharge-monney.detail({id:${id}, type: 'money'})">查看详情</a> `;
          return tpl;
        },
        sort: false,
        width: 10
